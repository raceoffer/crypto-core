...
const CompoundKey = require('./primitives/compoundkey');
const PaillierProver = require('./primitives/paillierprover');
const PaillierVerifier = require('./primitives/paillierverifier');
...
const Signer = require('./primitives/signer');
...
Marshal.wrap = function (data) {
  ...
  if (data instanceof EthereumTransaction) {
    return {
      ...
      signer: Marshal.wrap(data.signer)
    };
  }
  ...
  if (data instanceof jspaillier.publicKey) {
    return {
      ...
      bits: Marshal.wrap(data.bits),
      n: Marshal.wrap(data.n),
      n2: Marshal.wrap(data.n2),
      np1: Marshal.wrap(data.np1),
      rncache: Marshal.wrap(data.rncache)
    };
  }
  ...
  if (_.isArray(data)) {
    return _.map(data, arg => Marshal.wrap(arg));
  }

  if (_.isObject(data)) {
    return _.mapValues(data, arg => Marshal.wrap(arg));
  }
