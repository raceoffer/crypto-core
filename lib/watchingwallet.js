...
function WatchingWallet() {
  if(!(this instanceof WatchingWallet))
    return new WatchingWallet();
...
WatchingWallet.fromOptions = function fromOptions(options) {
  return new WatchingWallet().fromOptions(options);
};

WatchingWallet.toInternal = function(amount) {
  return bcoin.amount.fromBTC(amount).value;
};

WatchingWallet.fromInternal = function(amount) {
  return bcoin.amount.btc(parseInt(amount));
};

WatchingWallet.addressFromScript = function(script) {
  return bcoin.address.fromScript(script);
};

Object.setPrototypeOf(WatchingWallet.prototype, EventEmitter.prototype);

WatchingWallet.prototype.load = async function load() {
...
WatchingWallet.load = function load(options) {
  return WatchingWallet.fromOptions(options).load();
};
