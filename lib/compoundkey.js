...
function CompoundKey(options) {
  ...
  // Own keyring (full)
  this.localPrivateKeyring = null;

  // External keyring (public only)
  this.remotePublicKeyring = null;

  // Compound keyring (public only)
  this.compoundPublicKeyring = null;
...
CompoundKey.bcoin = (typeof bcoin !== 'undefined') ? bcoin : null;

CompoundKey.set = function (bcoin) {
  CompoundKey.bcoin = bcoin;
  return CompoundKey;
};

CompoundKey.generateKeyring = function generateKeyring() {
  const n3 = ec.n.div(new BN(3));
  let key = null;
  do {
    key = ec.genKeyPair().getPrivate();
  } while(key.cmp(n3) > 0);
  return CompoundKey.bcoin.keyring.fromKey(key.toArrayLike(Buffer, 'be', 32));
};

CompoundKey.generate = function generate() {
  return CompoundKey.fromOptions({ localPrivateKeyring: CompoundKey.generateKeyring() });
};

CompoundKey.keyringFromSecret = function keyringFromSeed(secret) {
  return CompoundKey.bcoin.keyring.fromPrivate(secret);
};

CompoundKey.fromSeed = function fromSeed(seed) {
  return CompoundKey.fromOptions({ localPrivateKeyring: CompoundKey.keyringFromSeed(seed) });
};

CompoundKey.prototype.fromOptions = function fromOptions(options) {
  assert(options.localPrivateKeyring && options.localPrivateKeyring.privateKey,"A private keyring is required");

  this.localPrivateKeyring = options.localPrivateKeyring;

  if(options.remotePublicKeyring && options.remotePublicKeyring.publicKey) {
    this.remotePublicKeyring = options.remotePublicKeyring;
  }
...
CompoundKey.prototype.getPrivateKey = function getPrivateKey(enc) {
  return this.localPrivateKeyring.getPrivateKey(enc);
};

CompoundKey.prototype.getPublicKey = function getPublicKey(enc) {
  if(!this.remotePublicKeyring){
    return null;
  }
  return this.remotePublicKeyring.getPublicKey(enc);
};

CompoundKey.prototype.getCompoundPublicKey = function getCompoundPublicKey(enc) {
  if(!this.remotePublicKeyring){
    return null;
  }
  return this.compoundPublicKeyring.getPublicKey(enc);
};

CompoundKey.prototype.getCompoundKeyAddress = function getCompoundKeyAddress(enc) {
  if(!this.remotePublicKeyring){
    return null;
  }
  return this.compoundPublicKeyring.getKeyAddress(enc);
};

CompoundKey.prototype.startInitialCommitment = function startInitialCommitment() {
  assert(this.localPrivateKeyring, "The key must be initialized to start a commitment");

  return new PaillierProof.Prover({
    x:  ec.keyFromPrivate(this.localPrivateKeyring.privateKey).getPrivate(),
...
CompoundKey.prototype.finishInitialSync = function finishInitialSync(syncData) {
  const remoteKeyring = CompoundKey.bcoin.keyring.fromPublic(Buffer.from(syncData.Q.encode(true, 'array')));
  const point = syncData.Q;

  const key = ec.keyFromPrivate(this.localPrivateKeyring.privateKey).getPrivate();
  const compound = point.mul(key);

  this.remotePublicKeyring = remoteKeyring;
  this.remotePrivateCiphertext = syncData.c;
  this.remotePaillierPublicKey = syncData.pk;

  this.compoundPublicKeyring = CompoundKey.bcoin.keyring.fromPublic(Buffer.from(compound.encode(true, 'array')));
};
...
Signer.prototype.fromOptions = function fromOptions(options) {
  ...
  const key = ec.keyFromPrivate(this.compoundKey.getPrivateKey());
...
Signer.prototype.computeCiphertext = function computeCiphertext() {
  ...
  const x = ec.keyFromPrivate(this.compoundKey.getPrivateKey()).getPrivate();
...
Signer.prototype.extractSignature = function extractSignature(ciphertext) {
  ...
  return new Signature({ r: this.r, s: s });
};
