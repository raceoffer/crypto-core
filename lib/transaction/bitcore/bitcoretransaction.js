...
function BitcoreTransaction() {
  ...
  this.PublicKeyHashInput = null;
  this.PublicKeyInput = null;
...
BitcoreTransaction.prototype.totalOutputs = function() {
  ...
  const transaformer = output => {
    return {
      ...
      value: new this.BN(output.satoshis)
    };
  };
...
BitcoreTransaction.prototype.estimateFee = function() {
  ...
  let fee = 0;
  fee += statistics.inputs.reduce((sum, input) => sum.add(input.value), new BN());
  fee -= statistics.outputs.reduce((sum, output) => sum.add(output.value), new BN());
  fee -= statistics.change.reduce((sum, change) => sum.add(change.value), new BN());
