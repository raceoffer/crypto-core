...
function SchnorrProof(options) {
  if(!(this instanceof SchnorrProof))
    return new SchnorrProof(options);
...
SchnorrProof.bcoin = (typeof bcoin !== 'undefined') ? bcoin : null;

SchnorrProof.set = function (bcoin) {
  SchnorrProof.bcoin = bcoin;
  return SchnorrProof;
};
...
SchnorrProof.prototype.fromSecret = function fromSecret(x) {
  ...
  this.t = ec.g.mul(r);
  this.c = new BN(SchnorrProof.bcoin.crypto.sha256(Buffer.from(this.t.encode(true))), 16);
...
SchnorrProof.prototype.verify = function verify(Q) {
  const c = new BN(SchnorrProof.bcoin.crypto.sha256(Buffer.from(this.t.encode(true))), 16);
  if(c.cmp(this.c) !== 0) {
    return false;
  }
