...
class NeoWallet {
  ...
  async getBalance() {
    ...
    let confirmed = new BigNumber(0);
    let unconfirmed = new BigNumber(0);

    const asset = data.assets[this.assetName];

    if (asset !== undefined) {
      confirmed = new BigNumber(asset.balance);
      unconfirmed = confirmed.plus(asset.unconfirmed.reduce((balance, value) => {
        return balance.plus(new BigNumber(value['value']));
      }, new BigNumber(0)));
    }
  ...
  async prepareTransaction(transaction, to, value, fee) {
    let assetAmounts = {};
    assetAmounts[this.assetName] = this.fromInternal(value).toNumber();
    ...
    const config = {
      ...
      intents: intent,
      fees: fee != undefined ? this.fromInternal(fee).toNumber() : 0
    };
