...
export class Signer {
  ...
  fromOptions(options) {
    ...
    this.curve = options.compoundKey.curve || 'ed25519';
    ...
    const rb = KeyPair.hash(this.compoundKey.localPrivateKey.messagePrefix, this.message);

    this.r = elliptic.utils.intFromLE(rb).umod(this.eddsa.curve.n);
  ...
  computePartialSignature() {
    ...
    const hb = KeyPair.hash(Rencoded, this.eddsa.encodePoint(this.compoundKey.compoundPublic), this.message);

    const h = utils.intFromLE(hb).mul(this.compoundKey.localPrivate);

    return {
      s: this.r.add(h).umod(this.eddsa.curve.n).toString(16)
    };
  }
