...
export class Signature {
  ...
  static fromOptions(options) {
    return new Signature().fromOptions(options);
  }
  ...
  static fromJSON(json) {
    return new Signature().fromJSON(json);
  }

  toHex() {
    return this.crypto.makeSignature({ R: this.R, S: this.S, Rencoded: this.crypto.encodePoint(this.R) }).toHex().toLowerCase();
  }
}
...
export class Signer {
  ...
  finalizeSignature(s) {
    const s2 = this.computePartialSignature();

    const S = s.s.add(s2.s).umod(this.crypto.curve.n);
