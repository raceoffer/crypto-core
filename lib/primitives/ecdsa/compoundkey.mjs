...
export class CompoundKey {
  ...
  static generateKey(curve = 'secp256k1') {
    const ecdsa = elliptic.ecdsa(curve);

    const n3 = ecdsa.n.div(new BN(3));
    let key = null;
    do {
      key = ecdsa.genKeyPair();
  ...
  fromOptions(options) {
    ...
    this.ecdsa = elliptic.ec(this.curve);
  ...
  importSyncData(syncData) {
    ...
    this.remotePublicKey = this.ecdsa.keyFromPublic(Buffer.from(point.encode(true, 'array')));
    this.remotePrivateCiphertext = syncData.c;
    this.remotePaillierPublicKey = syncData.pk;

    this.compoundPublicKey = this.ecdsa.keyFromPublic(Buffer.from(compound.encode(true, 'array')));
  }
