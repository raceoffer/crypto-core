...
const proto = require('./paillierkeys.json');
...
class PaillierPublicKey {
  ...
  toJSON(hex) {
    return {
      ...
      rncache: Array.isArray(this.publicKey.rncache) ? this.publicKey.rncache.map((x) => encodeBigInteger(x, hex)) : []
    };
  }
  ...
  fromJSON(json, hex) {
    this.publicKey = new PaillierPublicKeyProxy(
      ...
      Array.isArray(json.rncache) ? json.rncache.map((x) => decodeBigInteger(x, hex)) : []
    );
