...
DDS.prototype.getAddress = function (userId, accountId) {
  const seed = utils.deriveAesKey(Buffer.from(userId, 'utf8'));
  const chain = keychain.fromSeed(seed);
  const coin = 60;
  return this.accountFromSecret(chain.getAccountSecret(coin, accountId || 0));
}
...
DDS.prototype.estimateStoreGas = async function estimateStoreGas(options) {
  ...
  const userAddress = this.getAddress(options.id);
...
DDS.prototype.exists = async function (id) {
  const address = this.getAddress(id).address;
