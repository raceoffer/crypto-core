...
export class PaillierVerifier {
  ...
  processDecommitment(decommitment) {
    assert(PedersenScheme.verify(this.ec, this.remoteParams, this.aCommitment, decommitment));

    const Q = this.ec.curve.decodePoint(Buffer.from(decommitment.message,'hex'));

    assert(this.Q.mul(this.a).add(this.ec.g.mul(this.b)).eq(Q));
