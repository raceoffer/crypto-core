...
export class PaillierProver {
  ...
  getInitialCommitment() {
    const data = {
      ...
      Q: Buffer.from(this.ec.g.mul(this.x).encode(true)).toString('hex'),
  ...
  processInitialDecommitment(decommitment) {
    assert(PedersenScheme.verify(this.ec, this.remoteParams,this.iCommitment,decommitment));
    ...
    const options = {
      ...
      Q: this.ec.curve.decodePoint(Buffer.from(data.Q, 'hex'))
    };
  ...
  processCommitment(commitment) {
    ...
    const Q = this.ec.g.mul(this.alpha);
  ...
  processDecommitment(decommitment) {
    assert(PedersenScheme.verify(this.ec, this.remoteParams,this.sCommitment,decommitment));
